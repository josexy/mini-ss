...
func ResolveDefaultRouteInterface() (string, error) {
	// return ExeShell("route -n get default | grep 'interface' | awk 'NR==1{print $2}'")
	...
	for _, message := range msgs {
		...
		if _, ok := addresses[1].(*route.Inet4Addr); ok {
			// ok
			if iface, err := ResolveInterfaceByIndex(routeMessage.Index); err == nil {
				return iface.Name, nil
			}
		}
	}

	return "", fmt.Errorf("ambiguous gateway interfaces found")
}
